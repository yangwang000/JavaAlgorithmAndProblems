...
import com.java.adawang.datastructure.StackAndQueue.LinkedQueue;
...
public abstract class AbstractTree<E> implements Tree<E> {
	...
	public Iterable<Position<E>> breadthfirst(){
		...
		if(!isEmpty()){
			...
			((LinkedQueue<Position<E>>) fringe).enqueue(root());
			while(!fringe.isEmpty()){
				Position<E> p=
						((LinkedQueue<Position<E>>) fringe).dequeue();
				snapshot.add(p);
				for(Position<E> c: children(p))
					((LinkedQueue<Position<E>>) fringe).enqueue(c);
			}
		}
		return snapshot;
	}
